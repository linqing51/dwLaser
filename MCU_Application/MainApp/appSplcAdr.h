...
#define EM_DIODE_TEMP0							(EM_START + 35)//二极管温度0
#define EM_DIODE_TEMP1							(EM_START + 36)//二极管温度1
#define EM_DRIVE_TEMP							(EM_START + 37)//驱动器温度
#define EM_ENVI_TEMP							(EM_START + 38)//环境温度
#define EM_DIODE_HIGH_TEMP						(EM_START + 39)//二极管过热阈值
#define EM_DRIVE_HIGH_TEMP						(EM_START + 40)//驱动器过热阈值
#define EM_ENVI_HIGH_TEMP						(EM_START + 41)//环境过热阈值
/*****************************************************************************/
#define EM_EPID0_TAB_OUT						(EM_START + 42)//EPID输出值地址
#define EM_EPID0_TAB_REF						(EM_START + 43)//EPID设置定值
#define EM_EPID0_TAB_VFB						(EM_START + 44)//EPID反馈值
#define EM_EPID0_TAB_KP							(EM_START + 45)//EPID比例常数 XX.XXX 0-65.535
#define	EM_EPID0_TAB_KI							(EM_START +	46)//EPID积分常数 XX.XXX 0-65.535		
#define EM_EPID0_TAB_TD 						(EM_START + 47)//EPID微分时间常数 XX.XXX 0-65.535
#define EM_EPID0_TAB_TS							(EM_START + 48)//EPID运算时间间隔 1-3000 单位0.01S
#define EM_EPID0_TAB_EN_FP32L					(EM_START +	49)//EPID当前误差值 FP32 
#define EM_EPID0_TAB_EN_FP32H					(EM_START +	50)//EPID当前误差值 FP32 
#define EM_EPID0_TAB_VFB1_FP32L 				(EM_START +	51)//EPID前1次误差值 FP32
#define EM_EPID0_TAB_VFB1_FP32H					(EM_START +	52)//EPID前1次误差值 FP32
#define EM_EPID0_TAB_MAX						(EM_START + 53)//EPID输出最大限制		
#define EM_EPID0_TAB_MIN						(EM_START + 54)//EPID输出最小限制
#define EM_EPID0_TAB_BIAS						(EM_START +	55)//EPID输出偏置
#define EM_EPID0_TAB_TSC						(EM_START + 56)//EPID间隔计时器
#define EM_EPID0_TAB_KP_SUM_FP32L				(EM_START +	57)//EPID比例累加值 FP32
#define EM_EPID0_TAB_KP_SUM_FP32H				(EM_START +	58)//EPID比例累加值 FP32
#define EM_EPID0_TAB_KI_SUM_FP32L				(EM_START +	59)//EPID微分累加值 FP32
#define EM_EPID0_TAB_KI_SUM_FP32H				(EM_START +	60)//EPID微分累加值 FP32
#define EM_EPID0_TAB_KD_SUM_FP32L				(EM_START +	61)//EPID微分累加值 FP32
#define EM_EPID0_TAB_KD_SUM_FP32H				(EM_START +	62)//EPID微分累加值 FP32
#define EM_EPID0_TAB_FOUT_FP32L					(EM_START +	63)//EPID输出累加值 FP32
#define EM_EPID0_TAB_FOUT_FP32H					(EM_START +	64)//EPID输出累加值 FP32
#define EM_LASER_CURRENT_0						(EM_START +	65)//激光器电流0
#define EM_LASER_CURRENT_1						(EM_START +	66)//激光器电流1
#define EM_HMI_OPERA_STEP						(EM_START +	67)//操作步骤	
