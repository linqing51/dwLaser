...
#define FSMSTEP_CHECK_SPI_FLASH						204//SPI FLSAH自检
#define FSMSTEP_CHECK_CH376							205//USBHOST模块自检
#define FSMSTEP_CHECK_TEMPER						206//温度自检
#define FSMSTEP_CHECK_NFC							207//NFC模块自检
#define FSMSTEP_CHECK_NRF24L01						208//无线脚踏RF自检
#define FSMSTEP_CHECK_LDR							209//激光驱动器自检
...
void dcHmiLoop(void){//HMI轮训程序
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_POWERUP){//上电步骤	
		...
		RES(R_CHECK_NFC_PN7150B0HN_PASS);				
		...
		RES(R_CHECK_NFC_PN7150B0HN_FAIL);				
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_RESTORE_HMI){//等待HMI复位
		...
		if(LD(T_100MS_START * 16 + T100MS_HMI_POWERUP_DELAY)){
			...
			SetTextValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_TEXTDISPLAY_CHECK_INFO, "SELF-CHECK:Processor Pass");
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_EPROM){//EPROM自检步骤
		...
		if(LD(T_100MS_START * 16 + T100MS_CHECK_EPROM_DELAY)){
			...
			if(LD(R_CHECK_EPROM_PASS) && LDB(R_CHECK_EPROM_FAIL)){
				...
				SetTextValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_TEXTDISPLAY_CHECK_INFO, "Check System NVRAM Pass");
				SetProgressValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_PROGRESS_CHECK_INFO, 10);
			}
			else{
				SetTextValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_TEXTDISPLAY_CHECK_INFO, "Check System NVRAM Fail!");
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_SI7060){//温度传感器自检步骤
		...
		if(LD(T_100MS_START * 16 + T100MS_CHECK_SI7060_DELAY)){
			...
			if(LDB(SPCOIL_SI7060_INIT_FAIL)){
				...
				SetTextValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_TEXTDISPLAY_CHECK_INFO, "SELF-CHECK:SI7060 Pass");
				SetProgressValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_PROGRESS_CHECK_INFO, 20);
			}
			else{
				RES(R_CHECK_SI7060_PASS);
				SET(R_CHECK_SI7060_FAIL);
				SetTextValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_TEXTDISPLAY_CHECK_INFO, "SELF-CHECK:SI7060 Fail!");
				NVRAM0[EM_HMI_OPERA_STEP] = FSMSTEP_CHECK_FAIL_DISPLAY;//EPROM自检失败
			}	
		}
		else{
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_MCP79412){
		...
		if(LD(T_100MS_START * 16 + T100MS_CHECK_MCP79412_DELAY)){
			...
			if(LDB(SPCOIL_MCP79412_INIT_FAIL)){
				...
				SetTextValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_TEXTDISPLAY_CHECK_INFO, "SELF-CHECK:MCP79412 Pass");
				SetProgressValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_PROGRESS_CHECK_INFO, 30);
			}
			else{
				RES(R_CHECK_MCP79412_PASS);
				SET(R_CHECK_MCP79412_FAIL);
				SetTextValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_TEXTDISPLAY_CHECK_INFO, "SELF-CHECK:MCP79412 Fail");
				NVRAM0[EM_HMI_OPERA_STEP] = FSMSTEP_CHECK_FAIL_DISPLAY;//EPROM自检失败
			}	
		}
		else{
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_SPI_FLASH){
		...
		if(LD(T_100MS_START * 16 + T100MS_CHECK_SPI_FLASH_DELAY)){
			...
			if(LDB(SPCOIL_MCP79412_INIT_FAIL)){
				...
				RES(R_CHECK_SPI_FLASH_FAIL);
				NVRAM0[EM_HMI_OPERA_STEP] = FSMSTEP_CHECK_SPI_FLASH;					
				SetTextValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_TEXTDISPLAY_CHECK_INFO, "SELF-CHECK:Backup Memory Pass");
				SetProgressValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_PROGRESS_CHECK_INFO, 40);
			}
			else{
				RES(R_CHECK_SPI_FLASH_PASS);
				SET(R_CHECK_SPI_FLASH_FAIL);
				SetTextValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_TEXTDISPLAY_CHECK_INFO, "SELF-CHECK:Backup Memeory Fail");
				NVRAM0[EM_HMI_OPERA_STEP] = FSMSTEP_CHECK_FAIL_DISPLAY;//EPROM自检失败
			}	
		}
		else{
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_CH376){
		T100MS(T100MS_CHECK_USB_CH376_DELAY, true, CONFIG_CHECK_DELAY_TIME);
		if(LD(T_100MS_START * 16 + T100MS_CHECK_USB_CH376_DELAY)){
			T100MS(T100MS_CHECK_USB_CH376_DELAY, false, CONFIG_CHECK_DELAY_TIME);
			SET(R_CHECK_CH376_DONE);
			if(LDB(SPCOIL_CH376_INIT_FAIL)){
				SET(R_CHECK_CH376_PASS);
				RES(R_CHECK_CH376_FAIL);
				NVRAM0[EM_HMI_OPERA_STEP] = FSMSTEP_CHECK_SPI_FLASH;					
				SetTextValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_TEXTDISPLAY_CHECK_INFO, "SELF-CHECK:USB HOST Pass");
				SetProgressValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_PROGRESS_CHECK_INFO, 40);
			}
			else{
				RES(R_CHECK_CH376_PASS);
				SET(R_CHECK_CH376_FAIL);
				SetTextValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_TEXTDISPLAY_CHECK_INFO, "SELF-CHECK:USB HOST Fail");
				NVRAM0[EM_HMI_OPERA_STEP] = FSMSTEP_CHECK_FAIL_DISPLAY;//EPROM自检失败
			}	
		}
		else{
