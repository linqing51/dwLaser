...
void dcHmiLoop(void){//HMI轮训程序
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_PROCESSOR){//处理器自检步骤
		if(floatSumTest()){//测试成功
			...
			SetProgressValue(GDDC_PAGE_POWERUP, GDDC_PAGE_POWERUP_PROGRESS_CHECK_INFO, 5);	
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_BACKUP_FLASH){
		...
		if(LD(T_100MS_START * 16 + T100MS_CHECK_SPI_FLASH_DELAY)){
			...
			SET(R_CHECK_SPI_FLASH_DONE);
			if(LDB(SPCOIL_MCP79412_INIT_FAIL)){
				SET(R_CHECK_SPI_FLASH_PASS);
				RES(R_CHECK_SPI_FLASH_FAIL);
