...
static void updateLanguage(uint16_t lang){//更新语言
	SetLanguage(NVRAM0[DM_LANGUAGE], 1);
}
...
void dcHmiLoop(void){//HMI轮训程序
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_SAFETY_INTERLOCK){
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_WIRE_FOOTCONTROL){
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_FIBER_SENSOR){
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_WIRELESS_FOOTCONTROL){
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_PROBATION){
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_WAIT_ENTER_PASSCODE){//
		...
		case GDDC_PAGE_OPTION:{//选项页面
			switch(control_id){
				...
				case GDDC_PAGE_OPTION_KEY_LANGUAGE_EN:{
					if(state == 0x01){
						NVRAM0[DM_LANGUAGE] = DEF_LANG_EN;
						updateLanguage();//更新语言
					}
					else if(state == 0x0){
					}
				...
				case GDDC_PAGE_OPTION_KEY_LANGUAGE_CN:{
					if(state == 0x01){
						NVRAM0[DM_LANGUAGE] = DEF_LANG_CN;
						updateLanguage();
					}
					else if(state == 0x0){
					}
