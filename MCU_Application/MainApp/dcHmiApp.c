...
static uint8_t StandbyEnableState;//Standby按键状态
static uint8_t StandbyTouchState;
void UpdateUI(void);
...
void updateWarnMsgDisplay(uint8_t id){//更新警号显示框
	...
	if(MsgId != id){
		...
		switch(NVRAM0[EM_LASER_PULSE_MODE]){
			case LASER_MODE_CW:{
				SetTextValue(GDDC_PAGE_STANDBY_CW_0, GDDC_PAGE_STANDBY_CW_TEXTDISPLAY_WARN, pstr);
				break;
			}
			case LASER_MODE_SP:{
				SetTextValue(GDDC_PAGE_STANDBY_SP_0, GDDC_PAGE_STANDBY_SP_TEXTDISPLAY_WARN, pstr);
				break;
			}
			case LASER_MODE_MP:{
				SetTextValue(GDDC_PAGE_STANDBY_MP_0, GDDC_PAGE_STANDBY_MP_TEXTDISPLAY_WARN, pstr);
				break;
			}
			case LASER_MODE_GP:{
				SetTextValue(GDDC_PAGE_STANDBY_GP_0, GDDC_PAGE_STANDBY_GP_TEXTDISPLAY_WARN, pstr);
				break;
			}
			case LASER_MODE_SIGNAL:{
				SetTextValue(GDDC_PAGE_STANDBY_SIGNAL_0, GDDC_PAGE_STANDBY_SIGNAL_TEXTDISPLAY_WARN, pstr);
			}
			case LASER_MODE_DERMA:{
				SetTextValue(GDDC_PAGE_STANDBY_DERMA_0, GDDC_PAGE_STANDBY_DERMA_TEXTDISPLAY_WARN, pstr);
				break;
			}
			default:break;
		}
	...
	if(StandbyTouchState != enable){
		switch(NVRAM0[EM_LASER_PULSE_MODE]){
			case LASER_MODE_CW:{
				...
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_POWER_CH0_ADD, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_POWER_CH0_DEC, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_POWER_CH1_ADD, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_POWER_CH1_DEC, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_MODE_CW, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_MODE_MP, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_MODE_GP, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_MODE_SP, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_MODE_SIGNAL, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_MODE_DERMA, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_ENTER_OPTION, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_ENTER_SCHEME, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_SCHEME_SAVE, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_SCHEME_NEXT, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_SCHEME_LAST, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_SELECT_CH0, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_SELECT_CH1, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_SELECT_BOTH, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_KEY_RESET, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_PROGRESS_CH0, enable);
				BatchSetEnable(GDDC_PAGE_STANDBY_CW_PROGRESS_CH1, enable);
				BatchEnd();
				break;
			}
			case LASER_MODE_SP:{
			BatchBegin(GDDC_PAGE_STANDBY_SP_0);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_POWER_CH0_ADD, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_POWER_CH0_DEC, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_POWER_CH1_ADD, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_POWER_CH1_DEC, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_MODE_CW, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_MODE_MP, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_MODE_GP, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_MODE_SP, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_MODE_SIGNAL, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_MODE_DERMA, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_ENTER_OPTION, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_ENTER_SCHEME, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_SCHEME_SAVE, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_SCHEME_NEXT, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_SCHEME_LAST, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_SELECT_CH0, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_SELECT_CH1, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_SELECT_BOTH, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_KEY_RESET, enable);
			
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_PROGRESS_CH0, enable);
			BatchSetEnable(GDDC_PAGE_STANDBY_SP_PROGRESS_CH1, enable);
...
void standbyKeyEnable(uint8_t ena){//Standby使能
	if(StandbyEnableState != ena){
		...
		switch(EM_LASER_PULSE_MODE){
			case LASER_MODE_CW:{
				SetControlEnable(GDDC_PAGE_STANDBY_CW_0, GDDC_PAGE_STANDBY_CW_KEY_STANDBY, ena);
				break;
			}
			case LASER_MODE_SP:{
				SetControlEnable(GDDC_PAGE_STANDBY_SP_0, GDDC_PAGE_STANDBY_SP_KEY_STANDBY, ena);
				break;
			}
			case LASER_MODE_MP:{
				SetControlEnable(GDDC_PAGE_STANDBY_MP_0, GDDC_PAGE_STANDBY_MP_KEY_STANDBY, ena);
				break;
			}
			case LASER_MODE_GP:{
				SetControlEnable(GDDC_PAGE_STANDBY_GP_0, GDDC_PAGE_STANDBY_GP_KEY_STANDBY, ena);
				break;
			}
			case LASER_MODE_SIGNAL:{
				SetControlEnable(GDDC_PAGE_STANDBY_SIGNAL_0, GDDC_PAGE_STANDBY_SIGNAL_KEY_STANDBY, ena);
				break;
			}
			case LASER_MODE_DERMA:{
				SetControlEnable(GDDC_PAGE_STANDBY_DERMA_0, GDDC_PAGE_STANDBY_DERMA_KEY_STANDBY, ena);
				break;
			}
			default:break;
		}
...
void standbyKeyValue(uint8_t value){//Standby键值
	switch(EM_LASER_PULSE_MODE){
		case LASER_MODE_CW:{
			SetButtonValue(GDDC_PAGE_STANDBY_CW_0, GDDC_PAGE_STANDBY_CW_KEY_STANDBY, value);
			break;
		}
		case LASER_MODE_SP:{
			SetButtonValue(GDDC_PAGE_STANDBY_SP_0, GDDC_PAGE_STANDBY_SP_KEY_STANDBY, value);
			break;
		}
		case LASER_MODE_MP:{
			SetButtonValue(GDDC_PAGE_STANDBY_MP_0, GDDC_PAGE_STANDBY_MP_KEY_STANDBY, value);
			break;
		}
		case LASER_MODE_GP:{
			SetButtonValue(GDDC_PAGE_STANDBY_GP_0, GDDC_PAGE_STANDBY_GP_KEY_STANDBY, value);
			break;
		}
		case LASER_MODE_SIGNAL:{
			SetButtonValue(GDDC_PAGE_STANDBY_SIGNAL_0, GDDC_PAGE_STANDBY_SIGNAL_KEY_STANDBY, value);
			break;
		}
		case LASER_MODE_DERMA:{
			SetButtonValue(GDDC_PAGE_STANDBY_DERMA_0, GDDC_PAGE_STANDBY_DERMA_KEY_STANDBY, value);
			break;
		}
		default:break;
	}
...
void dcHmiLoop(void){//HMI轮训程序
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_STANDBY){//待机状态机
		...
		if(LD(R_FAULT)){
			...
			if(LDN(X_ESTOP)){//急停按下
				updateWarnMsgDisplay(1);		
			}
			else if(LDN(X_INTERLOCK)){//安全连锁拔出
				updateWarnMsgDisplay(1);
			}
			else if(LDN(X_FBD0)){//光纤拔出
				updateWarnMsgDisplay(1);
			}
			else if(LDN(X_FOOTSWITCH_NC)){//脚踏拔出
				updateWarnMsgDisplay(1);
			}
			else if(LDN(R_FIBER_ID_PASS_0)){//光纤ID不匹配
				updateWarnMsgDisplay(1);
			}
		...
		if(LD(R_STANDBY_KEY_STNADBY_DOWN)){//点击READY	
			...
			else if(LDB(R_FAULT) && LDB(X_FOOTSWITCH_NO)){//无故障
				standbyTouchEnable(false);
				standbyKeyEnable(false);
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_READY_ERROR){//Ready检测到脚踏踩下
		if(LDB(X_FOOTSWITCH_NO)){
			//停止蜂鸣器
			//Standby按钮恢复到Standby状态
			//使能Standby按钮
			//回到Standby状态
			NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_STANDBY;
		}
