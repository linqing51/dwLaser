		...
		if(LD(R_STANDBY_KEY_POSWIDTH_ADD_DOWN)){//正脉宽加按键
			...
			if(LD(T_10MS_START * 16 + T10MS_POSWIDTH_ADD_KEYDOWN_DELAY)){	
				if(LDP(SPCOIL_PS10MS) && (NVRAM0[EM_LASER_POSWIDTH] < CONFIG_MAX_LASER_POSWIDTH)){
					...
					SetTextValue(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_TEXTDISPLAY_POSWIDTH, dispBuf);	
				}
			}
		}
		...
		if(LD(R_STANDBY_KEY_NEGWIDTH_ADD_DOWN)){//负脉宽加按键
			...
			if(LD(T_10MS_START * 16 + T10MS_NEGWIDTH_ADD_KEYDOWN_DELAY)){	
				if(LDP(SPCOIL_PS10MS) && (NVRAM0[EM_LASER_NEGWIDTH] < CONFIG_MAX_LASER_NEGWIDTH)){
					...
					SetTextValue(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_TEXTDISPLAY_NEGWIDTH, dispBuf);
				}
			}
		}
	...
	switch(screen_id){
		...
		case GDDC_PAGE_STANDBY:{//主界面STANDBY
			switch(control_id){
				case GDDC_PAGE_STANDBY_KEY_POWER_CH0_ADD:{
					if(state == 0x01){
						if(NVRAM0[EM_LASER_POWER_CH0] < CONFIG_MAX_LASERPOWER_CH0){
							...
							SetTextValue(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_TEXTDISPLAY_POWER_CH0, dispBuf);
							sprintf(dispBuf,"%4.1f W", ((fp32_t)(NVRAM0[EM_TOTAL_POWER]) / 10));
							SetTextValue(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_TEXTDISPLAY_TOTAL_POWER, dispBuf);		
							SetProgressValue(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_PROGRESS_CH0, ((uint32_t)NVRAM0[EM_LASER_POWER_CH0] * 100 / CONFIG_MAX_LASERPOWER_CH0));						
						}
					}
					break;
				...
				case GDDC_PAGE_STANDBY_KEY_POWER_CH1_ADD:{
					if(state == 0x01){
						if(NVRAM0[EM_LASER_POWER_CH1] < CONFIG_MAX_LASERPOWER_CH1){
							...
							SetTextValue(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_TEXTDISPLAY_POWER_CH1, dispBuf);
							sprintf(dispBuf,"%4.1f W", ((fp32_t)(NVRAM0[EM_TOTAL_POWER]) / 10));
							SetTextValue(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_TEXTDISPLAY_TOTAL_POWER, dispBuf);
							SetProgressValue(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_PROGRESS_CH1, ((uint32_t)NVRAM0[EM_LASER_POWER_CH1] * 100 / CONFIG_MAX_LASERPOWER_CH1));
						}
				...
				case GDDC_PAGE_STANDBY_KEY_TIMES_ADD:{
					if(state == 0x00){
						RES(R_STANDBY_KEY_TIMES_ADD_DOWN);
						...
						RES(R_STANDBY_KEY_TIMES_ADD_UP);
						if(NVRAM0[EM_LASER_TIMES] < CONFIG_MAX_LASER_TIMES){
							ADDS1(EM_LASER_TIMES);//+1
							SetTextInt32(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_TEXTDISPLAY_TIMES ,NVRAM0[EM_LASER_TIMES], 1, 0);
						}
					}
					break;
				}
				case GDDC_PAGE_STANDBY_KEY_TIMES_DEC:{
					if(state == 0x0){
						RES(R_STANDBY_KEY_TIMES_DEC_DOWN);
						...
						RES(R_STANDBY_KEY_TIMES_DEC_UP);
						if(NVRAM0[EM_LASER_TIMES] > CONFIG_MIN_LASER_TIMES){
							DECS1(EM_LASER_TIMES);//-1
							SetTextInt32(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_TEXTDISPLAY_TIMES ,NVRAM0[EM_LASER_TIMES], 1, 0);
						}	
					}
					break;
				}
				case GDDC_PAGE_STANDBY_KEY_GROUP_OFF_ADD:{
					if(state == 0x0){
						RES(R_STANDBY_KEY_GROUP_OFF_ADD_DOWN);
						SET(R_STANDBY_KEY_GROUP_OFF_UP);
					}
					else if(state == 0x01){//DOWN
						SET(R_STANDBY_KEY_GROUP_OFF_DOWN);
						RES(R_STANDBY_KEY_GROUP_OFF_UP);
						if(NVRAM0[EM_LASER_GROUP_OFF] < CONFIG_MAX_LASER_GROUP_OFF){
							ADDS1(EM_LASER_GROUP_OFF);//+1
							sprintf(dispBuf,"%d mS", NVRAM0[EM_LASER_GROUP_OFF]);
							SetTextValue(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_TEXTDISPLAY_GROUP_OFF, dispBuf);
						}
					}
					break;
				}
				case GDDC_PAGE_STANDBY_KEY_GROUP_OFF_DEC:{
					if(state == 0x0){//UP
						RES(R_STANDBY_KEY_GROUP_OFF_DEC_DOWN);
						...
						RES(R_STANDBY_KEY_GROUP_OFF_DEC_UP);
						if(NVRAM0[EM_LASER_GROUP_OFF] > CONFIG_MIN_LASER_GROUP_OFF){
							DECS1(EM_LASER_GROUP_OFF);//-1
							sprintf(dispBuf,"%d mS", NVRAM0[EM_LASER_GROUP_OFF]);
							SetTextValue(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_TEXTDISPLAY_GROUP_OFF, dispBuf);
						}
					}
