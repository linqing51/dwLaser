...
static void standbyKeyEnable(uint8_t ena){//Standby使能
...
static void standbyKeyValue(uint8_t value){//Standby键值
	switch(EM_LASER_PULSE_MODE){
		case LASER_MODE_CW:{
...
void dcHmiLoop(void){//HMI轮训程序
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_STANDBY){//待机状态机
		...
		if(LD(R_FAULT)){
			//打开蜂鸣器报警
			if(LD(R_DCHMI_KEY_STANDBY_ENABLE)){
				RES(R_DCHMI_KEY_STANDBY_ENABLE);
			...
			if(LD(R_DCHMI_KEY_STANDBY_ENABLE)){
				...
				SET(R_DCHMI_KEY_STANDBY_ENABLE);
			}
			BeemEnable = 0;
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSETP_READY_ERROR){//Ready检测到脚踏踩下
