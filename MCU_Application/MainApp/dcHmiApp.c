...
void dcHmiLoop(void){//HMI轮训程序
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_POWERUP){//上电步骤	
		...
		if(NVRAM0[DM_DC_OLD_PASSCODE0] == 0x0000){
			NVRAM0[DM_DC_OLD_PASSCODE0] = 0x3433;//00
		}
		if(NVRAM0[DM_DC_OLD_PASSCODE1] == 0x0000){
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_CHECK_EPROM){//EPROM自检步骤
		...
		if((NVRAM0[TM_START + 0] == NVRAM0[TM_START + 4]) && 
		   (NVRAM0[TM_START + 1] == NVRAM0[TM_START + 5]) &&
		   (NVRAM0[TM_START + 2] == NVRAM0[TM_START + 6]) &&
		   (NVRAM0[TM_START + 3] == NVRAM0[TM_START + 7])){
