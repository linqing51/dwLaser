...
void dcHmiLoop(void){//HMI轮训程序
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_STANDBY){//待机状态机
		...
		else if(LD(R_DIODE_TEMP_HIGH_1)){
			SetTextValue(GDDC_PAGE_STANDBY, GDDC_PAGE_STANDBY_TEXTDISPLAY_WARN, LANG_WARN_MSG_DIODE1_HTEMP[NVRAM0[DM_LANGUAGE]]);
		}
		...
		else if(R_DRIVE_TEMP_HIGH){//驱动器过热
	...
	if(NVRAM0[EM_HMI_OPERA_STEP] == FSMSTEP_DIAGNOSIS){
		if(LD(R_DIAGNOSIS_OK_DOWN)){//返回Option
			...
			NVRAM0[EM_HMI_OPERA_STEP] = FSMSTEP_DIAGNOSIS;
			if(LD(X_INTERLOCK)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_INTERLOCK , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_INTERLOCK , 0, 1, 0);
			}
			if(LD(X_ESTOP)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_ESTOP , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_ESTOP , 0, 1, 0);
			}
			if(LD(X_FOOTSWITCH_NO)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_FOOTSWITCH_NO , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_FOOTSWITCH_NO , 0, 1, 0);
			}
			if(LD(X_FOOTSWITCH_NC)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_FOOTSWITCH_NC , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_FOOTSWITCH_NC , 0, 1, 0);
			}
			if(LD(X_LDR_FAULT0)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_LDR_FAULT0 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_LDR_FAULT0 , 0, 1, 0);
			}
			if(LD(X_LDR_FAULT1)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_LDR_FAULT1 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_LDR_FAULT1 , 0, 1, 0);
			}
			if(LD(X_FBD0)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_FBD0 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_FBD0 , 0, 1, 0);
			}
			if(LD(X_FBD1)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_FBD1 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_X_FBD1 , 0, 1, 0);
			}
			if(LD(Y_LED_POWERON)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_LED_POWERON , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_LED_POWERON , 0, 1, 0);
			}
			if(LD(Y_LED_EMIT)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_LED_EMIT , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_LED_EMIT , 0, 1, 0);
			}
			if(LD(Y_LED_ALARM)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_LED_ALARM , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_LED_ALARM , 0, 1, 0);
			}
			if(LD(Y_OUT3)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y3 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y3 , 0, 1, 0);
			}
			if(LD(Y_OUT4)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y4 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y4 , 0, 1, 0);
			}
			if(LD(Y_OUT5)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y5 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y5 , 0, 1, 0);
			}
			if(LD(Y_OUT6)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y6 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y6 , 0, 1, 0);
			}
			if(LD(Y_OUT7)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y7 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y7 , 0, 1, 0);
			}
			if(LD(Y_FAN0)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_FAN0 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_FAN0 , 0, 1, 0);
			}
			if(LD(Y_FAN1)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_FAN1 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_FAN1 , 0, 1, 0);
			}
			if(LD(Y_TEC0)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_TEC0 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_TEC0 , 0, 1, 0);
			}
			if(LD(Y_TEC1)){
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_TEC1 , 1, 1, 0);
			}
			else{
				SetTextInt32(GDDC_PAGE_DIAGNOSIS, GDDC_PAGE_DIAGNOSIS_TEXTDISPLAY_Y_TEC1 , 0, 1, 0);
			}
